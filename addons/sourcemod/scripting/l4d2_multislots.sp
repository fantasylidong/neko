...
ConVar g_hSLimit, g_hNaxHeal, g_hpills, g_hgive0, g_hgive1, g_hgive2, g_hgive3, g_hgive4, g_hgive5;
ConVar g_check, g_haway, g_hdaze, g_hkick, g_hsset, g_hmaxs, g_hLimit, g_hTeam, g_health;

int userids, maxplayers;
int hgive0, hgive1, hgive2, hgive3, hgive4, hgive5;
int iCheck, iDaze, iAway, iKick, iSset, iMaxs, hLimit, hTeam, health;

Handle g_TimerSpecCheck = null, g_hBotsUpdateTimer = null;

bool g_bShouldFixAFK, g_bRoundStarted, gbVehicleLeaving, gbFirstItemPickedUp, l4d2_sv_maxplayers, l4d2_Check_Interval;
...
int g_iBotPlayer[MAXPLAYERS+1], ClientSpawnMaxTimer[MAXPLAYERS+1], iVariableSurvivor[MAXPLAYERS+1], iNumPrinted[MAXPLAYERS+1], iDelayedValidationStatus[MAXPLAYERS+1];
...
public void OnPluginStart()
{
	...
	g_hNaxHeal		= FindConVar("first_aid_kit_max_heal");
	g_hpills		= FindConVar("pain_pills_health_threshold");
	...
	g_check			= CreateConVar("l4d2_multislots_enabled_afk_check",				"10",		"设置幸存者发呆多长时间后提示 !afk 闲置指令可用/秒(最小值:10). 0=禁用.", CVAR_FLAGS);
	...
	g_health		= CreateConVar("l4d2_multislots_enabled_player_health",			"100",		"设置幸存者的血量上限(默认值:100)(最低值:100).", CVAR_FLAGS);
	...
	g_health.AddChangeHook(l4d2OtherConVarChanged);
...
void l4d2GetOtherCvars()
{
	...
	iCheck	= g_check.IntValue;
	
	if (iCheck > 0 && iCheck < 10)
		iCheck = 10;
	
	iDaze	= g_hdaze.IntValue;
	iAway	= g_haway.IntValue;
	iKick	= g_hkick.IntValue;
	iSset	= g_hsset.IntValue;
	iMaxs	= g_hmaxs.IntValue;
	...
	g_hNaxHeal.IntValue	= health = g_health.IntValue;
	
	if (health < 100)
		health = 100;
	
	g_hpills.IntValue	= health - 1;
	
	if (!l4d2_sv_maxplayers)
	{
		SetConVarInt(FindConVar("sv_maxplayers"), iMaxs, false, false);
		SetConVarInt(FindConVar("sv_visiblemaxplayers"), iMaxs, false, false);
	}
	else
	{
		...
		if (GetClientTeam(client) == 2)
		{
			...
			if (MoveTheMouse[client])
			{
				...
				if (GazeMovement(client))
				{
					...
					if (iVariableSurvivor[client] >= iDaze) 
					{
						SDKCall(hGoAwayFromKeyboard, client);
						iVariableSurvivor[client] = 0;
						ClientTimerDaze[client] = null;
						//PrintToChatAll("\x04[提示]\x03%N\x05计时结束,执行闲置.", client, iDaze);
						return Plugin_Stop;
					}
				}
				else
...
public void Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsValidClient(client))
		SetEntProp(client, Prop_Data, "m_iMaxHealth", health);//设置幸存者血量.
}

void vGiveDefaultItems(int client)
{
	vRemovePlayerWeapons(client);//玩家离开后清理过于武器和物品.
	PlayeriMaxHealth(client);//设置幸存者血量.
...
void PlayeriMaxHealth(int client)
{
	SetEntProp(client, Prop_Data, "m_iHealth", health);//设置幸存者血量.
	SetEntProp(client, Prop_Data, "m_iMaxHealth", health);//设置幸存者血量上限.
}
...
void vRoundStartiMaxHealth()
{
	for (int i = 1; i <= MaxClients; i++)
		if(IsValidClient(i))
			SetEntProp(i, Prop_Data, "m_iMaxHealth", health);//设置幸存者血量.
}
...
void DirectorHint(int client, const char[] sHintCaption)
{
	...
	FormatEx(sValues, sizeof(sValues), "%f", iCheck - 2.0);
	...
	FormatEx(sValues, sizeof(sValues), "OnUser1 !self:Kill::%f:1", iCheck - 1.0);
...
{
	g_bRoundStarted = true;
	vRoundStartiMaxHealth();//回合开始时设置血量上限.
...
void l4d2_GiveWeapon_pistol_2(int client)
{
	switch(GetRandomInt(0,7))
	{
		...
		case 3:
			BypassAndExecuteCommand(client, "give", "cricket_bat");//板球棒
		case 4:
			BypassAndExecuteCommand(client, "give", "frying_pan");//平底锅
		case 5:
			BypassAndExecuteCommand(client, "give", "electric_guitar");//电吉他
		case 6:
			BypassAndExecuteCommand(client, "give", "crowbar");//铁撬棍
		case 7:
			BypassAndExecuteCommand(client, "give", "weapon_chainsaw");//电锯
	}
}
